	...
	fun Player.openPaginatedMenu(title: String, items: List<GuiItem>, titleItems: List<GuiItem> = listOf()) =
		Tasks.sync {
			...
			for (page in 0 until pages) {
				...
				if (pages != 1) {
					val button =
						guiButton(ItemStack(Material.WHITE_STAINED_GLASS_PANE)).setName("Page ${page + 1}/$pages")
				...
				if (page != 0) {
					...
					topBar.addItem(
						...
						}.setName("Previous Page (${previous + 1})"),
						0, 0
					)
				}
